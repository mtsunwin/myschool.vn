 ...
 */
class AdapterDataListNoti(var context: Context, var listTitle: ArrayList<mNotification>) : BaseAdapter() {


    override fun getView(position: Int, converview: View, p2: ViewGroup?): View {
        var view: View
        var viewHolder: listHolder
        if (converview != null) {
            var layoutInflater = LayoutInflater.from(context)
            view = layoutInflater.inflate(R.layout.layout_item_list_notification, p2, false)
            viewHolder = listHolder(view)
            view.tag = viewHolder
        } else {
            view = converview
            viewHolder = converview!!.tag as listHolder
        }
